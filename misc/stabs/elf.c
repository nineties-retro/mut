/*
 *.intro: A simple dump program for static ELF executables.
 *
...
static Elf32_Shdr * locate_stab_header(void)
{
	Elf32_Ehdr * header= (Elf32_Ehdr *)file_contents;
	size_t max= header->e_shnum;
	size_t n= 1;
	...
	for (n= 1; n < max; n += 1) {
		...
		if (section_header->sh_type == SHT_PROGBITS) {
			char const * name= locate_section_header_string(section_header->sh_name);
...
static int display_symbols(void)
{
	for (; *symbol_names; symbol_names+=1) {
		unsigned long hash= elf_hash(*symbol_names);
		printf("XXX %s=%lu/%lu/%lu/%lu\n", *symbol_names, hash, hash%188, hash%189, hash%190);
	}
	return EXIT_SUCCESS;
}
...
static int display_symbol_table(void)
{
	...
	symbol_table_header= locate_symbol_table_header();
	...
	for (i= 1; n != 0; n-=sizeof(Elf32_Sym), i+=1, symbol+=1) {
		printf("SYM%u (%p) name=%lu (%s), addr=%lx, shndx=%u\n", i, symbol, symbol->st_name, locate_string(symbol->st_name), symbol->st_value, symbol->st_shndx);
	}
	return EXIT_SUCCESS;
}


static int display_stabs(void)
{
	Elf32_Shdr * stab_header= locate_stab_header();
	char * xxx;
	assert(stab_header);
	xxx = &file_contents[stab_header->sh_offset];
	return EXIT_FAILURE;
}
...
static int debug(void)
{
	...
	if (display_stabs() == EXIT_FAILURE)
		goto could_not_display_stabs;
